...
public class XdpServer {
    ...
    public Future<Void> start(){
        if (!Platform.isLinux()){
            return Future.failedFuture("XDP only supports linux");
        }
        SemVer semVer = Platform.linuxVer();
        if (SemVer.parse("6.6.14").compareTo(semVer) > 0){
            return Future.failedFuture("Kernel version is insufficiently up‑to‑date to support XDP: "+semVer);
        }
        try {
