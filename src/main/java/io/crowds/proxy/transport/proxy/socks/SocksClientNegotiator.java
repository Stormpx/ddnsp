...
public class SocksClientNegotiator extends SimpleChannelInboundHandler<SocksMessage> {
    ...
    public SocksClientNegotiator(Channel channel, Destination dst) {
    ...
    private void init(){
        channel.pipeline()
               .addLast("socks-encoder",Socks5ClientEncoder.DEFAULT)
               .addLast("socks-decoder",new Socks5InitialResponseDecoder())
               .addLast("socks-negotiator",this);
        this.promise=channel.eventLoop().newPromise();
    }

    private void cleanChannelHandler(){
        this.channel.pipeline().remove(Socks5ClientEncoder.class);
        this.channel.pipeline().remove(Socks5CommandResponseDecoder.class);
        this.channel.pipeline().remove(SocksClientNegotiator.class);
    }
    ...
    protected void channelRead0(ChannelHandlerContext ctx, SocksMessage msg) throws Exception {
        if (msg instanceof Socks5InitialResponse response){
            ...
            this.channel.pipeline().remove(Socks5InitialResponseDecoder.class);
            this.channel.pipeline().addBefore("socks-negotiator","socks-decoder",new Socks5CommandResponseDecoder());
